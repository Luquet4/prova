...
numbers_list = numbers_input.strip(',') # Não é Trim e sim Split - Esqueci como converter diretamente de string para int


# Calcula o cubo de cada número (erro: os elementos não foram convertidos para int)

squared_numbers = filter(lambda x: x * 3, numbers_list) # Cubo é 3 não 2

# Filtra apenas os cubos pares (erro: sintaxe incorreta no lambda)

even_squares = filter(lambda x: x % 2 == 0, squared_numbers) # escrita lamda ao inves de lambda e == para comparação

# Soma os cubos pares utilizando reduce (erro: sintaxe incorreta na chamada de reduce)

soma_total = reduce(lambda a, b: a + b, even_squares)  # Variavel nome errado , Reduce não importado corretamente - from functools import reduce ou import functools - ai seria functools.reduce() - Iteravel even_squares fora do reduce
